...
using Neo.SmartContract.Framework;
using Neo.SmartContract.Framework.Attr;
using Neo.SmartContract.Framework.Services;
using System;
...
{
    [SupportedStandards("NEP-11")]
    public class NFTTokenContract : Nep11Token<Nep11TokenState>
    {
         public class MyTokenState
        {
            public string Name = string.Empty;
            public string Description = string.Empty;
        }
        ...
        [InitialValue("NQAHgVNjQp4FcdgQFdDqp4sfeKY6Bq9M6s", ContractParameterType.Hash160)]
        ...
        public static bool Airdrop(UInt160 to, string name, string description)
        {
            if (!IsOwner()) throw new Exception("No authorization.");
            if (!to.IsValid) throw new Exception("Amount is invalid.");

            var tokenState = new MyTokenState
            {
                Name = name,
                Description = description,
            };

            Mint(name, tokenState);
            return true;
        }
    }
} */